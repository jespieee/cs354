...
/* Returns 1 if the value in the cell of a row/column is valid. Otherwise
 * returns 0. A valid row or column contains only blanks or the digits 1-size,
 * with no duplicate digits, where size is the value 1 to 9.
 *
 * digit: the value of the cell
 * size:  number of rows and columns in the board
 * seen:  an array of size 10 which stores the counts of each digit seen so far
 * in a row/column
 */
...
int valid_sudoku_board(int **board, int size) {
    ...
    for (int i = 0; i < size; i++) {
        // use an array of index 0 - 9 with values of 0 to keep track of
        // existing digits
